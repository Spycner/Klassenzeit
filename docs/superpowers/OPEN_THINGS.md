...
- **Uniform `/api` prefix on the backend.** Today the backend mounts routers at the root (`/auth/*`, `/subjects`, `/teachers`, `/rooms`, `/week-schemes`, `/stundentafeln`, `/classes`, `/lessons`, `/health`, `/openapi.json`, `/docs`, `/redoc`) and the Vite dev proxy enumerates each prefix. This bit the staging deployment (PR #88+): `server-infra/Caddyfile`'s staging block assumed `/api/*` for backend routes and everything else for the frontend, so the SPA fell back to `index.html` on `/health` + every API call. A temporary `path_regexp` matcher that enumerates every top-level backend segment now lives in `server-infra/Caddyfile` on the VPS (not yet committed to that repo) so the staging URL works. The proper fix, to plan in a fresh session: (1) add `prefix="/api"` to `include_router` calls for auth + scheduling in `backend/src/klassenzeit_backend/main.py`; move `/health`, `/openapi.json`, `/docs`, `/redoc` under `/api` as well (or decide which stay at root); (2) update all ~60 backend test path literals (`/auth/login` → `/api/auth/login`, etc.); (3) regen `frontend/src/lib/api-types.ts` via `mise run fe:types`, update every `client.METHOD('/...')` call site in `frontend/src/features/*/hooks.ts` to the new path; (4) collapse `frontend/vite.config.ts` proxy to a single `/api` rule; (5) revert `server-infra/Caddyfile` to the clean `handle /api/* { reverse_proxy ... }` pattern. Mechanical refactor, touches many files but each change is trivial.
