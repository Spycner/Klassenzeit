...
### Hessen Grundschule reference data (for the E2E smoke step)
...
- **Auto-assign teachers during generate-lessons (or in the solver).** `build_problem_json` filters `Lesson` by `teacher_id IS NOT NULL`, so lessons with no teacher assigned are invisible to the solver. The `generate-lessons` route creates `Lesson` rows with `teacher_id=None`, leaving the user to fill in a teacher per row before `POST /schedule` produces any placements. For the seeded Grundschule demo that is a jarring extra click. Options: (a) extend `generate-lessons` to pick a qualified teacher per subject while respecting `max_hours_per_week` (simple heuristic today; becomes first-fit-decreasing when FFD lands); (b) let the solver itself choose the teacher from the qualifications pool. Either way a separate PR. Surfaced during grundschule seed work.
- **Grundschule seed uses 7-period grid instead of 6 for greedy slack.** The MVP greedy solver is bin-packing-sensitive; extending the seeded WeekScheme to a 7-period day (08:00 to 14:05, Stunde 7 from 13:20 to 14:05) makes the einzügige Grundschule solvable today. The Hessen reference is a 6-period Halbtag. Revisit when FFD ordering or LAHC local search ship (see solver backlog items), so the seed can match the Halbtag shape. Surfaced during grundschule seed work.
...
- **Soft constraints with an objective function.** Hessen pedagogy has informal norms worth respecting: Hauptfächer (Deutsch, Mathematik) earlier in the day, no long gaps for the Klassenlehrer, Sport not first period and not directly after Essen, Musik/Kunst preferred in the dedicated rooms. Introduce `SolveConfig { weights: ConstraintWeights }` and an objective function that sums weighted penalties over the placement set. The greedy picks the lowest-delta placement per slot (archive/v2 `scheduler/src/construction.rs` is the reference). Soft-constraint land-and-tune only makes sense against the seed.
- **Local search optimisation phase (LAHC).** After the greedy terminates, run a late-acceptance hill-climb that swaps lesson placements to reduce the combined (hard_violations, soft_score) tuple. Reference: `scheduler/src/local_search.rs` on `archive/v2`. Prerequisites: FFD + soft constraints + scoring + a `SolveConfig` with budget and seed. Ship in a follow-up to those, not on its own.
- **`SolveConfig` struct on the public API.** Optional `deadline: Option<Duration>` for a best-so-far return under a wallclock budget, and `seed: u64` for deterministic tiebreaking once local search introduces randomness. Required prerequisite for LAHC. Adds a third public function `solve_with_config(&Problem, &SolveConfig)` without breaking the existing `solve(&Problem)` signature.
- **Structured violation taxonomy.** `ViolationKind::UnplacedLesson.message` is free-form today. Real runs against the seed will reveal which failure modes matter; replace with typed variants (`NoFreeTimeBlock`, `NoSuitableRoom`, `TeacherOverCapacity`, `NoRoomMeetsConstraint`, etc., mirroring archive/v2's `ViolationKind` enum where appropriate) so the schedule view can group, translate, and act on them. Requires a wire-contract change; coordinate with the frontend schedule view.
- **Criterion benchmarks for the solver.** Wire `mise run bench` (placeholder today) to `cargo bench -p solver-core` with a criterion benchmark that solves the Grundschule fixture several hundred times and records placements per second plus p50 / p99 latency. Add once the seed lands so benchmarks run against realistic inputs, not synthetic proptest generators. Regression budget: within 20% of the MVP baseline after FFD + LAHC ship.
