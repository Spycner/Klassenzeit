...
- **Branch protection has undocumented repo-settings prerequisites.** Applying `docs/superpowers/branch-protection.json` with `required_linear_history: true` fails with HTTP 422 unless the repo has at least one non-merge-commit strategy enabled. This was fixed out-of-band during Task 12 by setting `allow_squash_merge: true`, `allow_rebase_merge: true`, `allow_merge_commit: false` via `gh api --method PATCH /repos/:owner/:repo`. That repo-level change is not recorded in `branch-protection.json` (which only covers branch-scoped settings). A fresh repo re-applying the JSON would hit the same 422. Fix: add a companion `docs/superpowers/repo-settings.json` or fold the repo-settings PATCH into a wrapper script so the two states are applied together.
