...
The `# ty: ignore[missing-argument]` is because pydantic-settings reads fields from env at runtime; the project's type checker (Astral's `ty`) sees `database_url` as a required field. For call sites that also pass the private `_env_file` kwarg, add `unknown-argument` to the rule list as well — `ty` doesn't know about pydantic-settings' private init kwargs.
