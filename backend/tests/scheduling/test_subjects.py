...
async def test_create_subject(
    ...
    response = await client.post("/subjects", json={"name": "Mathematik", "short_name": "Ma"})
...
async def test_create_subject_duplicate_name(
    ...
    response = await client.post("/subjects", json={"name": "Deutsch", "short_name": "D2"})
...
async def test_list_subjects(
    ...
    await client.post("/subjects", json={"name": "Englisch", "short_name": "En"})
    await client.post("/subjects", json={"name": "Biologie", "short_name": "Bi"})
...
async def test_get_subject(
    ...
    create_resp = await client.post("/subjects", json={"name": "Chemie", "short_name": "Ch"})
...
async def test_get_subject_not_found(
    ...
    response = await client.get(f"/subjects/{uuid.uuid4()}")
...
async def test_update_subject(
    ...
    create_resp = await client.post("/subjects", json={"name": "Physik", "short_name": "Ph"})
    subject_id = create_resp.json()["id"]
    response = await client.patch(f"/subjects/{subject_id}", json={"name": "Physik neu"})
    assert response.status_code == 200
    body = response.json()
...
async def test_delete_subject(
    ...
    create_resp = await client.post("/subjects", json={"name": "Kunst", "short_name": "Ku"})
    subject_id = create_resp.json()["id"]
    delete_resp = await client.delete(f"/subjects/{subject_id}")
...
async def test_delete_subject_not_found(
    ...
    response = await client.delete(f"/subjects/{uuid.uuid4()}")
    assert response.status_code == 404
