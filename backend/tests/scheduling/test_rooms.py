...
async def test_create_room_duplicate_name(
    ...
    await client.post("/rooms", json={"name": "Dup Room", "short_name": "DR"})
    response = await client.post("/rooms", json={"name": "Dup Room", "short_name": "DR2"})
...
async def test_list_rooms(
    ...
    await client.post("/rooms", json={"name": "Alpha Room", "short_name": "AR"})
    await client.post("/rooms", json={"name": "Beta Room", "short_name": "BR"})
...
async def test_get_room_detail(
    ...
    # Create the room
    room_resp = await client.post("/rooms", json={"name": "Detail Room", "short_name": "DT"})
    room_id = room_resp.json()["id"]

    # Create a subject and assign as suitability
    subj_resp = await client.post("/subjects", json={"name": "Physics", "short_name": "PHY"})
    subject_id = subj_resp.json()["id"]
    await client.put(f"/rooms/{room_id}/suitability", json={"subject_ids": [subject_id]})

    # Create a week scheme + time block for availability
    ws_resp = await client.post("/week-schemes", json={"name": "Detail Week Scheme"})
    ...
    await client.put(f"/rooms/{room_id}/availability", json={"time_block_ids": [tb_id]})
...
async def test_update_room(
    ...
    room_resp = await client.post(
        "/rooms", json={"name": "Old Room Name", "short_name": "ORN", "capacity": 20}
    )
    room_id = room_resp.json()["id"]
    response = await client.patch(
        f"/rooms/{room_id}", json={"name": "New Room Name", "capacity": 25}
    )
    assert response.status_code == 200
    body = response.json()
...
async def test_delete_room(
    ...
    room_resp = await client.post("/rooms", json={"name": "To Delete Room", "short_name": "TDR"})
    room_id = room_resp.json()["id"]
    delete_resp = await client.delete(f"/rooms/{room_id}")
    assert delete_resp.status_code == 204
    get_resp = await client.get(f"/rooms/{room_id}")
...
async def test_replace_suitability(
    ...
    room_resp = await client.post("/rooms", json={"name": "Suit Room", "short_name": "SR"})
    room_id = room_resp.json()["id"]

    # Create two subjects
    math_resp = await client.post("/subjects", json={"name": "Mathematics", "short_name": "MAT"})
    chem_resp = await client.post("/subjects", json={"name": "Chemistry", "short_name": "CHE"})
    math_id = math_resp.json()["id"]
    chem_id = chem_resp.json()["id"]

    # Set initial suitability to math only
    first_put = await client.put(f"/rooms/{room_id}/suitability", json={"subject_ids": [math_id]})
    ...
    second_put = await client.put(f"/rooms/{room_id}/suitability", json={"subject_ids": [chem_id]})
    ...
    # Confirm via GET
    detail = await client.get(f"/rooms/{room_id}")
...
async def test_replace_availability(
    ...
    room_resp = await client.post("/rooms", json={"name": "Avail Room", "short_name": "AVR"})
    room_id = room_resp.json()["id"]

    # Create week scheme and two time blocks
    ws_resp = await client.post("/week-schemes", json={"name": "Avail Test Scheme"})
    ws_id = ws_resp.json()["id"]
    ...
    first_put = await client.put(
        f"/rooms/{room_id}/availability", json={"time_block_ids": [tb1_id, tb2_id]}
    )
    assert first_put.status_code == 200
    first_body = first_put.json()
    ...
    second_put = await client.put(
        f"/rooms/{room_id}/availability", json={"time_block_ids": [tb2_id]}
    )
    assert second_put.status_code == 200
    second_body = second_put.json()
