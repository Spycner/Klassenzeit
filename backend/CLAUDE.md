...
- **Middleware integration tests use `httpx.ASGITransport(app=build_app(env="dev"))`** + `AsyncClient` + `caplog.set_level(logging.INFO, logger="...")`. Pattern in `tests/test_http_access_middleware.py`.
